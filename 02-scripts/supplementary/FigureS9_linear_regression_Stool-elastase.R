...
# Your mapping table
mapping <- tribble(
  ~sample_information, ~stool_elastase,
  "PDM1", 16,
  "PDM2", 74,
  "PDM3", 242,
  "PDM4", NA,
...
# Add new column to your dataframe df
pdm_df <- pdm_df %>%
  left_join(mapping, by = "sample_information") %>%
  mutate(stool_elastase = replace_na(stool_elastase, 0))

median_nonzero <- median(pdm_df$stool_elastase[pdm_df$stool_elastase != 0])

#pdm_df$stool_elastase_imputed <- ifelse(
#  pdm_df$stool_elastase == 0, 
#  median_nonzero, 
#  pdm_df$stool_elastase
#)

#pdm_df <- pdm_df %>%
#  filter(!sample_information %in% c("PDM4", "PDM5", "PDM10", "PDM16", "PDM19"))

#pdm_df <- pdm_df %>%
#  mutate(stool_elastase_log = log1p(stool_elastase_imputed))

#p <- ggplot(pdm_df, aes(x = Blautia, y = `stool_elastase_log`, color = Type)) +
#  geom_point(size = 2) +
#  geom_smooth(method = "lm", se = TRUE) +
#  stat_poly_eq(
#    aes(label = paste(..p.value.label..)),
...
#    label.x.npc = "left", label.y.npc = 0.95
#  ) +
#  labs(title = "", x = "Escherichia-Shigella", y = "log(1 + Stool elastase)") +
#  scale_color_manual(values = c("DM" = "#E1812C", "T3cDM" = "#3A923A", "K" = "blue")) +
#  theme_minimal()
#p
############## K MEAN IMPUTATION 
...
# 1. Select numeric variables except stool_elastase
num_vars_no_elastase <- pdm_df %>% select(where(is.numeric), -stool_elastase)

# 2. Remove columns with zero variance
num_vars_no_elastase <- num_vars_no_elastase[, sapply(num_vars_no_elastase, function(x) sd(x, na.rm = TRUE) > 0)]

# 3. Impute missing values (simple: replace NA with column median)
num_vars_no_elastase <- num_vars_no_elastase %>%
  mutate(across(everything(), ~ ifelse(is.na(.), median(., na.rm = TRUE), .)))

# 4. Scale
num_scaled <- scale(num_vars_no_elastase)

# 5. Run k-means
set.seed(123)
km <- kmeans(num_scaled, centers = 3, nstart = 25)

pdm_df$cluster <- km$cluster
...
# Global median among non-zero values
global_median <- median(pdm_df$stool_elastase[pdm_df$stool_elastase > 0], na.rm = TRUE)

# Cluster medians (non-zero), with fallback to global if a cluster has no non-zero values
cluster_medians <- pdm_df %>%
  group_by(cluster) %>%
  summarise(median_nonzero = median(stool_elastase[stool_elastase > 0], na.rm = TRUE), .groups = "drop") %>%
  mutate(median_nonzero = ifelse(is.na(median_nonzero), global_median, median_nonzero))

# Impute zeros (and any NAs) using the cluster-specific median (with fallback already applied)
pdm_df <- pdm_df %>%
  left_join(cluster_medians, by = "cluster") %>%
  mutate(
    stool_elastase_imputed = ifelse(stool_elastase == 0 | is.na(stool_elastase),
                                    median_nonzero, stool_elastase),
    stool_elastase_log = log1p(stool_elastase_imputed)
  ) %>%
  select(-median_nonzero)

# Quick checks
sum(is.na(pdm_df$stool_elastase_imputed))   # should be 0
sum(is.na(pdm_df$stool_elastase_log))       # should be 0

library(dplyr); library(ggplot2); library(patchwork)

# Confirm no missing values left
sapply(pdm_df[c("stool_elastase_imputed","stool_elastase_log")], \(x) sum(is.na(x)))


pdm_df <- pdm_df %>%
  mutate(stool_elastase_imputed_flag = as.integer(stool_elastase != stool_elastase_imputed))
...
p <- ggplot(pdm_df, aes(x = Blautia, y = stool_elastase_log, color = Type)) +
  ...
  stat_poly_eq(aes(label = paste(..p.value.label..)),
               formula = y ~ x, parse = TRUE,
               label.x.npc = "left", label.y.npc = 0.95) +
  labs(x = "Escherichia-Shigella", y = "log(1 + Stool elastase)") +
  scale_color_manual(values = c("DM" = "#E1812C", "T3cDM" = "#3A923A", "K" = "blue")) +
  theme_minimal()
p

#ggsave(plot=p,"/data/scratch/kvalem/projects/2024/diabetes_microbe/05-results/figures/stool_elastase_Escherichia-Shigella.svg", height = 3, width = 5, dpi=300)
#ggsave(plot=p,"/data/scratch/kvalem/projects/2024/diabetes_microbe/05-results/figures/stool_elastase_Escherichia-Shigella.png", height = 3, width = 5, dpi=300)

########


# Example: Save df as CSV in a specific directory
write.csv(pdm_df, file = "/data/scratch/kvalem/projects/2024/diabetes_microbe/01-tables/tables_02/supplementary_tables/stool_elastase_Escherichia-Shigella.csv", row.names = FALSE)
